 ...
 */
	...
	$('#enrol_submit').click(function(){
		...
		str = $("input[name='paymentplan']:checked").val();
		
		if(!str){
			alert("Please choose your payment plan");
			$("#paymentplan_yes" ).focus();
			return false;
		} else if(str=="Yes"){
			str=$("input[name='paymentplan_option']:checked").val();
			if(!checkLength(str)){
				alert("Please choose one of payment plan");
				$('#paymentplan_option').show("slow");
				$("#paymentplan_option" ).focus();
				return false;
			}
		}
		...
		if(output.length >0){
		    ...
		    var payment =$("input[name='paymentplan']:checked").val();
			var payment_option=$("select[name='paymentplan_option'] option:selected").text();
			...
			if(matches==null){
				...
				if(current_enrol){
				 ...
				 if(!payment|| payment=="No"){
					...
					if(payment_option){
						payment += " School: "+payment_option;
					}
				 }
				 		 
		         $("#confirm_payment").text(payment);
	...
	$( "#cal_accommodation" ).datepicker({ dateFormat: 'dd/mm/yy',beforeShowDay: null,changeMonth: true,    changeYear: true});
		...
		$("#course_type").on('change', function() {
			...
			$("#certiistart,#certiiistart").hide("fast");
			var optionVal = $("#course_type").val();
			switch(optionVal){
			case "Certificate II Business":
			$("#certiistart").show("fast");
			$("#certiiistart").hide("fast");
			$("#weeks").val("21");
			break;
			case "Certificate III Business":
			$("#certiiistart").show("fast");
			$("#certiistart").hide("fast");
			$("#weeks").val("26");
			break;
		...
		$("#paymentplan_yes").click(function() { 
			$('#paymentplan_option').show("slow");			
		});			
		$("#paymentplan_no").click(function() { 
			$('#paymentplan_option').hide("slow");			
		});			
		...
		$("#addcourse").click(function(){
			var rowCount = $('#courses tr').length;
			 if (rowCount>2){
				alert("Only a maximum of 2 courses can be added" +
						"!!!!!!!!!!");
				return false;
			 }
	          ...
	          height: 400,
	   	      ...
	   	      buttons: {
	   	        "ADD": function() {
					...
					var optionVal = $("#course_type").val();
					var start_date;
					if(optionVal=="Certificate II Business"){
						start_date=$('#certiistart').val();
						
					} else if ( optionVal=="Certificate III Business"){
						start_date=$('#certiiistart').val();
